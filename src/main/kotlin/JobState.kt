...
class JobState<U: Any> @PublishedApi internal constructor(val value: U?, val interceptors: List<WorkerInterceptor> = emptyList()) {
     ...
     */
    inline fun require(crossinline predicate: (U) -> Boolean): JobState<U> = process(JobStatus.PermanentFailure,  predicate)
    inline fun require(msg: String, crossinline predicate: (U) -> Boolean): JobState<U> = process(JobStatus.PermanentFailure, predicate, msg)
     ...
     */
    inline fun execute(crossinline predicate: (U) -> Boolean): JobState<U> = process(JobStatus.TransientFailure, predicate)
    inline fun execute(msg: String, crossinline predicate: (U) -> Boolean): JobState<U> = process(JobStatus.TransientFailure, predicate, msg)
     ...
     */
    inline fun advanceIf(crossinline predicate: (U) -> Boolean): JobState<U> = process(JobStatus.Success, predicate)
    inline fun advanceIf(msg: String, crossinline predicate: (U) -> Boolean): JobState<U> = process(JobStatus.Success, predicate, msg)
     ...
     */
    inline fun sideEffect(crossinline fn: (U) -> Unit): JobState<U> = execute { fn(it!!); true }
    ...
    inline fun process(newStatus: JobStatus, crossinline predicate: (U) -> Boolean, msg: String? = null): JobState<U> {
